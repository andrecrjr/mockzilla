...
jsf.format("x-template", (schema: any) => {
  ...
  return template.replace(/\{\{?(\w+)\}?\}/g, (match, key) => {
    const value = generationContext.storedValues.get(key)
    if (value === undefined) {
      console.warn(`[schema-generator] Template variable '${key}' not found in context`)
...
function processTemplates(data: any, visited = new WeakSet()): any {
  ...
  if (typeof data === "string") {
    ...
    return data.replace(/\{\{?\$\.([\w.[\]]+)\}?\}/g, (match, path) => {
      ...
      if (value === undefined) {
        console.warn(`[schema-generator] Template reference '${match}' could not be resolved`)
 ...
 */
function deepReplaceTemplates(rootData: any, currentData: any = rootData, visited = new WeakSet()): any {
  // Prevent circular reference infinite loops
  ...
  if (typeof currentData === "string") {
    ...
    return currentData.map((item) => deepReplaceTemplates(rootData, item, visited))
