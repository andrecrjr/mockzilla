...
export const CreateMockArgs = z.object({
	...
	statusCode: HttpStatusCodeSchema.describe('HTTP status code to return'),
	folderId: z
		.string()
		.optional()
		.describe(
	...
	delay: NonNegativeIntegerSchema.optional().describe(
...
export const ManageMocksArgs = z.union([
	z.object({
	...
	z.object({
		...
		delay: NonNegativeIntegerSchema.optional(),
	}).superRefine((value, context) => {
		if (!value.folderId && !value.folderSlug) {
			context.addIssue({
				code: z.ZodIssueCode.custom,
				message: 'folderId or folderSlug is required',
				path: ['folderId'],
			});
		}
		if (value.response === undefined && !value.jsonSchema) {
			context.addIssue({
				code: z.ZodIssueCode.custom,
				message: 'response is required if jsonSchema is not provided',
				path: ['response'],
			});
		}
	}),
...
/**
 * Object-shaped schema used to advertise this action-based tool through MCP.
 *
 * The MCP SDK only serializes object schemas in `tools/list`; registering the
 * discriminated union directly produces an empty JSON Schema. The handler still
 * validates requests against `ManageMocksArgs`, which preserves the per-action
 * required fields and constraints.
 */
export const ManageMocksInputSchema = z.object({
	action: z.enum(['list', 'create', 'get', 'update', 'delete', 'preview']),
...
export type ManageMocksInputSchema = z.infer<typeof ManageMocksInputSchema>;
