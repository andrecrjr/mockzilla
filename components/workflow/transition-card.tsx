...
import { ArrowRight, Copy, MoreHorizontal, Pencil, Trash } from 'lucide-react';
		...
		<Card className="p-4 group hover:border-primary/50 transition-colors">
			...
			<div className="space-y-2 text-sm">
				...
				<div className="flex items-start gap-2 text-muted-foreground">
					<span className="text-xs uppercase font-semibold text-muted-foreground/50 w-16 shrink-0 mt-0.5">
						When
					</span>
					<div className="flex-1">
						{Object.keys(transition.conditions || {}).length > 0 ? (
							<code className="text-xs bg-yellow-500/10 text-yellow-500 px-1 py-0.5 rounded">
								{JSON.stringify(transition.conditions).substring(0, 40)}
								{JSON.stringify(transition.conditions).length > 40 ? '...' : ''}
							</code>
						) : (
						...
						{(transition.effects || []).length > 0 ? (
							<>
								...
								<span className="text-muted-foreground font-normal ml-auto text-xs">
									{(transition.effects as unknown[]).length} rows
								</span>
							</>
						) : (
							<span className="text-muted-foreground/50 italic">
