...
describe("Mock Serving — wildcard variants", () => {
  ...
  it("serves variant when capture key matches", async () => {
    const wildcardMock = {
      ...
      variants: [
        { key: "123", body: JSON.stringify({ id: 123, name: "Alice" }), statusCode: 200, bodyType: "json" },
        { key: "456", body: JSON.stringify({ id: 456, name: "Bob" }), statusCode: 200, bodyType: "json" },
      ],
  ...
  it("serves correct variant for different capture key", async () => {
    const wildcardMock = {
      ...
      useDynamicResponse: false,
      variants: [
        { key: "123", body: JSON.stringify({ id: 123 }), statusCode: 200, bodyType: "json" },
        { key: "456", body: JSON.stringify({ id: 456 }), statusCode: 200, bodyType: "json" },
      ],
      wildcardRequireMatch: false,
    };
    ...
    const req = new NextRequest(
      "http://localhost:3000/api/mock/api/users/456"
    );
  ...
  it("serves variant with different statusCode", async () => {
    const wildcardMock = {
      ...
      useDynamicResponse: false,
      variants: [
        { key: "404user", body: JSON.stringify({ error: "not found" }), statusCode: 404, bodyType: "json" },
        { key: "500user", body: JSON.stringify({ error: "server error" }), statusCode: 500, bodyType: "json" },
      ],
      wildcardRequireMatch: false,
    };
  ...
  it("handles multi-segment capture keys in variants", async () => {
    const wildcardMock = {
      ...
      useDynamicResponse: false,
      variants: [
        { key: "alice|active", body: JSON.stringify({ user: "alice", status: "active" }), statusCode: 200, bodyType: "json" },
        { key: "bob|inactive", body: JSON.stringify({ user: "bob", status: "inactive" }), statusCode: 200, bodyType: "json" },
      ],
      wildcardRequireMatch: false,
    };
    ...
    const params = Promise.resolve({ path: ["api", "users", "bob", "status", "inactive"] });
