...
<div className="flex gap-4">
  ...
  <div>
    ...
    <div className="text-muted-foreground">Go to **Workflows** and create a new scenario named `Counter Demo`.</div>
  </div>
</div>
        </div>
      </div>
...
<div className="flex gap-4">
  ...
  <div>
    ...
    <div className="text-muted-foreground">Open the **State Inspector** and set a variable `count` to `0`.</div>
  </div>
</div>
      </div>
...
<div className="flex gap-4">
  ...
  <div>
    ...
    <div className="text-muted-foreground">Create a new transition:</div>
...
<div>
  <h4>Relational Lookups</h4>
  <div>
    <div>You can query your database directly in your responses or effects using predicates:</div>
    <ul className="list-disc ml-6 space-y-1">
      <li><code>{{db.users[id=1].name}}</code> - Finds user with ID 1.</li>
      <li><code>{{db.cart.[0].id}}</code> - Gets the ID of the first item in the cart.</li>
    </ul>
  </div>
</div>
...
<div className="text-muted-foreground mb-8 max-w-2xl mx-auto">
  Now that you know the basics, dive into one of our full-scale tutorial to see how these concepts build real-world systems.
</div>
