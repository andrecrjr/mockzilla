...
export default function ScenarioDetailPage() {
  ...
  const { data: transitions = [], isLoading, mutate: mutateTransitions } = useSWR<Transition[]>(
    ...
    <div className="container mx-auto py-8 max-w-7xl h-[calc(100vh-4rem)] flex flex-col">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 flex-1 min-h-0">
          ...
          <div className="flex-1 overflow-y-auto pr-2 space-y-4">
            {isLoading ? (
              ...
              <div className="text-center py-12 border-2 border-dashed rounded-lg bg-muted/10">
                <p className="text-muted-foreground">No transitions defined yet.</p>
                <p className="text-xs text-muted-foreground/50 mt-1">Use the button above to add one.</p>
              </div>
            ) : (
              transitions.map((t) => (
        ...
        <div className="lg:col-span-1 flex flex-col min-h-0">
          <StateInspector data={stateData} onRefresh={() => mutateState()} isLoading={false} />
        </div>
      </div>
