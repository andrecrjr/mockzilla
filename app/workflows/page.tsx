...
async function createScenario(url: string, { arg }: { arg: { name: string; description?: string } }) {
...
export default function WorkflowsPage() {
  ...
  const { data: scenarios, error, isLoading, mutate } = useSWR<Scenario[]>(
    '/api/workflow/scenarios', 
    fetcher
  );

  const { trigger, isMutating } = useSWRMutation('/api/workflow/scenarios', createScenario, {
    onSuccess: (data) => {
  ...
  const handleCreate = async () => {
    ...
    await trigger({ name: newScenarioName, description: newScenarioDescription || undefined });
  };
              ...
              <Card className="hover:border-primary/50 transition-colors cursor-pointer h-full">
                <div className="p-6">
                  ...
                  <p className="text-xs text-muted-foreground">
                    ID: <code className="bg-muted px-1 py-0.5 rounded">{scenario.id}</code>
                  </p>
                </div>
              </Card>
            </Link>
