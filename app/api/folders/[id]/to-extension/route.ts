...
export async function GET(
	...
	try {
		...
		const extensionMocks: ExtensionMock[] = mocks.map((mock) => {
			...
			if (originalMock) {
				...
				return {
					...
					body: mock.response,
					variants:
						variants.length > 0 ? variants : originalMock.variants || [],
					wildcardRequireMatch:
						wildcardRequireMatch || originalMock.wildcardRequireMatch || false,
				};
			}
			return {
				id: mock.id,
		...
		return NextResponse.json(
			{
				error:
					error instanceof Error ? error.message : 'Failed to export folder',
			},
			{ status: 500 },
		);
	}
